...
class assign_adminController {
	...
	private function showUser() {
		if(isset($_POST['user'])) {
			...
			if($user!='') {
				$sql = "SELECT * FROM users WHERE (username LIKE '%$user%' OR email LIKE '%$user%') AND is_social <> '1' ";
				$this->registry->getObject('db')->executeQuery($sql);
				$i = 0;
				while($row = $this->registry->getObject('db')->getRows()) {
					$i = 1;
					$this->registry->getObject('template')->buildFromTemplates( ['assign_admin/show-user.html'], $i);
	...
	private function assign($email, $username) {
		try {
			...
			if(!$admin && !$super_admin) {
				$query = "SELECT * FROM  `backend_priority` WHERE  `key` =  'assign_admin'";
